...
<!--创建一个名为nuclear的新目录，用于存放项目文件。
安装一些依赖库，如el-button、el-upload等，用于实现页面的按钮、上传等功能。
创建一个名为template的标签，用于定义页面的布局和样式。
在template标签内，使用el-breadcrumb、el-table、el-dialog等组件，用于显示面包屑导航、表格、弹窗等元素。
在template标签外，创建一个script标签，用于编写页面的逻辑和交互。
在script标签内，定义一些变量和方法，如data、handleSelectionChange、updateSongImg等，用于存储数据、处理选择变化、更新歌曲图片等操作。
使用axios库发送请求到后端服务器，获取或更新歌曲数据。-->

<!--导入axios库，用于发送请求。
定义data变量，用于存储页面的数据，如歌曲列表、搜索词、播放图标等。
定义created方法，用于在页面加载时获取歌曲列表。
定义handleSelectionChange方法，用于处理用户选择表格行时的变化。
定义deleteAll方法，用于批量删除选中的歌曲。
定义attachImageUrl方法，用于拼接歌手图片的完整地址。
定义setSongUrl方法，用于设置当前播放的歌曲，并切换播放图标。
定义parseLyric方法，用于解析歌词字符串为数组。
定义handleImgSuccess方法，用于处理更新图片成功后的回调。
定义handleSongSuccess方法，用于处理更新歌曲成功后的回调。
定义beforeImgUpload方法，用于在上传图片前检查文件类型和大小。
定义beforeSongUpload方法，用于在上传歌曲前检查文件类型和大小。-->